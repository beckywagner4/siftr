...
#'
#' @description This is a function to read data that has already been processed by the SYFT into three tables - analyte concentrations, concentrations per reagent and concentrations per product.
#'
...
read_proc_sift = function(file){
  ...
  read_data = function(start, end){
    ...
    title = suppressWarnings(readr::read_csv(file = file, col_types = readr::cols(),
                                             skip = start-1, n_max = 1, col_names = F)) %>%
      dplyr::pull(1) %>%
      gsub("\\..*","",.) %>%
      janitor::make_clean_names()

    df = suppressWarnings(readr::read_csv(file, col_types = readr::cols(),
                                          skip = start, n_max = end - start,
                                          locale = readr::locale(encoding = "ISO-8859-1"))) %>%
      dplyr::mutate(table = title,
  ...
  conc_per_product = raw[[3]] %>%
    tidyr::separate(name, into = c("product_ion", "product_ion_mass", "reagent_ion", "compound", "numbers", "unit"),
             sep = " \\[|  / | / | \\(") %>%
    dplyr::mutate(dplyr::across(where(is.character), stringr::str_remove, "\\)|\\]"))
